...
//         }
//       }
//       if (!uploaded) throw new Error(`Failed to upload ${remoteFile} after 3 attempts`);
//     }
//   }
// }
...
// function processRendition(filePath, outputPath, rendition, lessonId, renditionProgress) {
...
//         const totalPercent = Object.values(renditionProgress).reduce((a, b) => a + b, 0);
//         const overallProgress = totalPercent / renditions.length;

//         lessonProgress[lessonId] = parseFloat(overallProgress.toFixed(2));
//         console.log(`📊 Lesson ${lessonId} overall progress: ${lessonProgress[lessonId]}%`);
//       })
//       .on("end", () => {
...
// agenda.define("transcode-video", { concurrency: 2, lockLifetime: 1000 * 60 * 60 }, async (job) => {
//   const { filePath, lessonId } = job.attrs.data;
//   const outputPath = path.resolve(__dirname, "uploads", "courses", lessonId);
//   fs.mkdirSync(outputPath, { recursive: true });
...
//     await Promise.all(renditions.map((r) => processRendition(filePath, outputPath, r, lessonId, renditionProgress)));
