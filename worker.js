...
async function uploadFolderToMinIO(localDir, remoteDir, onProgress) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      uploadedFiles++;
      if (onProgress)
        onProgress({ uploaded: uploadedFiles, total: totalFiles });
    }
  }
}
...
agenda.define(
  "transcode-video",
  { concurrency: 1, lockLifetime: 1000 * 60 * 60 },
  async (job) => {
    ...
    const updateProgress = async (percent, stage) => {
      ...
      console.log(
        `📊 Progress updated: ${safePercent.toFixed(2)}%, Stage: ${stage}`
      );
    };
      ...
      await new Promise((resolve, reject) => {
        ffmpeg(filePath)
          ...
          .on("progress", async (prog) => {
            const percent =
              i * perRenditionProgress +
              ((prog.percent || 0) / 100) * perRenditionProgress;
